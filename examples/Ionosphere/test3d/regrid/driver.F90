program driver
  ...
  integer,parameter :: isv = 0, itype = 1
  ...
  do concurrent (i = 1:nlon, j = 1:nlat, t = 1:ntime, ivar = 1:nvar)
    var_ext_alt(i,j,:,t,ivar) = interp1d(alt,z(i,j,:,t),var_ext(i,j,:,t,ivar))
  enddo
...
! this might not be necessary
  ...
  do concurrent (inode = 1:nnode, k = 1:nalt, t = 1:ntime)
    b2 = b(inode,k,1)**2+b(inode,k,2)**2+b(inode,k,3)**2

    v = (/var_int(inode,k,t,ivx),var_int(inode,k,t,ivy),var_int(inode,k,t,ivz)/)
    vperp = v-dot_product(v,b(inode,k,:))*b(inode,k,:)/b2
