...
pde['tau'] = numpy.array([1.0]); # HDG stabilization parameter
...
pde['NLtol'] = 1e-2; # nonlinear solver tolerance
pde['NLiter'] = 15; # number of nonlinear iterations
pde['matvectol'] = 1e-10; # matrix-vector product tolerance

flagGencode = True

# Physical parameters
dt0 = 1.0  # time step size in seconds
...
for icell in range(ne):
    for inode in range(npe):
        ...
        mesh['udg'][inode, 1, icell] = u1[k, n]
        mesh['udg'][inode, 2, icell] = u2[k, n]
        mesh['udg'][inode, 3, icell] = u3[k, n]
        mesh['udg'][inode, 4, icell] = tn[k, n]
